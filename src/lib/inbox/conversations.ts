 ...
 * `update_conversation_review_state()` (migration 081), which stamps
 * this same pair of columns automatically whenever the reviewer sends
 * a message instead. Lets a human clear a conversation out of "Sem
 * supervisão" after just reading it, without needing to reply or take
 * the conversation over. Clara stays enabled either way — this only
 * touches the review timestamp, never `ai_autoreply_disabled`.
 */
